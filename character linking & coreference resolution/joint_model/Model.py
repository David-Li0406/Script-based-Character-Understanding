...
class JointLearningMemoryModel(nn.Module):
    def __init__(self, pre_trained, num_memory_layers, coref_weight=1, linking_weight=1, in_batch_weight=0, recap_weight=0):
        ...
        self.in_batch_weight = in_batch_weight
        self.recap_weight = recap_weight
    ...
    def forward(self, batch_of_segments, token_masks, mentions_seg, mention_start, mention_end, speaker_ids,
                link_first_mention_index=None, link_second_mention_index=None, character_label=None,
                recap_ids=None, recap_mention_start=None, recap_mention_end=None, recap_character_ids=None):
        # Get recap character embedding
        if recap_ids is not None:
            recap_mention_representations = None
            if recap_mention_start.size(0) != 0:
                recap_hiddens = self.encoder(input_ids=recap_ids, return_dict=True).last_hidden_state[0]
                recap_start_representation = recap_hiddens[recap_mention_start]
                recap_end_representation = recap_hiddens[recap_mention_end]
                recap_mention_representations = recap_start_representation + recap_end_representation
                unsqueezed_recap_mention_representations = torch.unsqueeze(recap_mention_representations, 0)
                for _ in range(self.num_memory_layers):
                    layer_outputs = self.mention_self_attention(unsqueezed_recap_mention_representations)
                    unsqueezed_recap_mention_representations = layer_outputs[0]
                recap_mention_representations = torch.squeeze(unsqueezed_recap_mention_representations, 0)
                # print(recap_mention_representations.shape)
                # print(recap_character_ids)
        
        outputs = self.encoder(input_ids=batch_of_segments, attention_mask=token_masks, return_dict=True)
        ...
        if link_first_mention_index is None:
            ...
            in_batch_loss, recap_loss, contrastive_size = 0., 0., 0
            if recap_ids is not None:
                #contrastive loss
                in_batch_loss, recap_loss, contrastive_size = self.compute_contrastive_loss(
                    mention_representations,
                    recap_mention_representations,
                    character_label,
                    recap_character_ids,
                )
            ...
            self.in_batch_weight * in_batch_loss + self.recap_weight * recap_loss, \
            contrastive_size
    ...
    def compute_contrastive_loss(
        ...
        ids2character_dialogue, ids2character_recap = {}, {}

        for pos, ids in enumerate(character_ids):
            ids = ids.item()
            if ids not in ids2character_dialogue:
                ids2character_dialogue[ids] = []
            ids2character_dialogue[ids].append(pos)
        
        for pos, ids in enumerate(recap_character_ids):
            ids = ids.item()
            if ids not in ids2character_recap:
                ids2character_recap[ids] = []
            ids2character_recap[ids].append(pos)

        # in batch contrastive loss
        z1, z2 = None, None
        for ids, poss in ids2character_dialogue.items():
            # print(poss)
            if len(poss) >= 2:
                pos1, pos2 = random.sample(poss,2)
                sample1, sample2 = mention_representations[pos1], mention_representations[pos2]
                if z1 is None:
                    z1 = sample1.unsqueeze(0)
                    z2 = sample2.unsqueeze(0)
                else:
                    z1 = torch.cat((z1, sample1.unsqueeze(0)))
                    z2 = torch.cat((z2, sample2.unsqueeze(0)))
        
        in_batch_loss, in_batch_size = 0., 0
        if z1 is not None:
            cos_sim = self.sim(z1.unsqueeze(1), z2.unsqueeze(0))
            labels = torch.arange(cos_sim.size(0)).long().to('cuda')
            in_batch_loss = loss_fct(cos_sim, labels)
            in_batch_size = labels.size(0)

        recap_loss, recap_size = 0., 0
        if recap_mention_representations is not None:
            # recap dialogue contrastive loss
            z1, z2 = None, None
            for ids, poss1 in ids2character_recap.items():
                if ids in ids2character_dialogue:
                    poss2 = ids2character_dialogue[ids]
                    pos1, pos2 = random.sample(poss1,1)[0], random.sample(poss2, 1)[0]
                    sample1, sample2 = recap_mention_representations[pos1], mention_representations[pos2]
                    if z1 is None:
                        z1 = sample1.unsqueeze(0)
                        z2 = sample2.unsqueeze(0)
                    else:
            ...
            if z1 is not None:
                cos_sim = self.sim(z1.unsqueeze(1), z2.unsqueeze(0))
                labels = torch.arange(cos_sim.size(0)).long().to('cuda')
                recap_loss = loss_fct(cos_sim, labels)
                recap_size = labels.size(0)

        return in_batch_loss, recap_loss, in_batch_size+recap_size
    ...
    def __init__(self, data_path, tokenizer):
        ...
        with open(data_path, "r") as fin:
            ...
            for line in tqdm(fin):
                ...
                # Added
                recap_ids = [document["recap_tokens"]]
                recap_mention_start = [m[0] for m in document["recap_mentions"]]
                recap_mention_end = [m[1]-1 for m in document["recap_mentions"]]
                recap_character_ids = [m[2] for m in document["recap_mentions"]]
                recap_ids = torch.tensor(recap_ids)
                recap_mention_start = torch.tensor(recap_mention_start)
                recap_mention_end = torch.tensor(recap_mention_end)
                recap_character_ids = torch.tensor(recap_character_ids)
                ...
                self.all_documents.append([batch_ids, masks, mention_seg, mention_start, mention_end, speaker_ids, link_start, link_end, character_ids, recap_ids, recap_mention_start, recap_mention_end, recap_character_ids])
